...
{
	int status, datalen, sd, *ip_flags, iCount, iGesamt;
  	const int on = 1;
	...
	Daten aus dem File laden und ein einen Buffer laden
	...
	while(1){
		...
		// switch(iCount){
			...
			// filein = fopen("image.jpg","rb+"); //black 1
				...
				// break;
			// case 1:
			// filein = fopen("imagew.jpg","rb+"); //white 1
				// strcpy (src_ip, "192.168.0.88");
				// break;
			// case 2:
			// filein = fopen("image1.jpg","rb+"); //black 2
				// strcpy (src_ip, "192.168.0.77");
				// break;
			// case 3:
			// filein = fopen("imagew1.jpg","rb+"); //white 2
				// strcpy (src_ip, "192.168.0.88");
				// break;
			// case 4:
			// filein = fopen("image2.jpg","rb+"); //black 3
				// strcpy (src_ip, "192.168.0.77");
				// break;
			// case 5:
			// filein = fopen("imagew2.jpg","rb+"); //white 3
				// strcpy (src_ip, "192.168.0.88");
				// break;
		...
		fclose(filein);
  		...
  		iphdr.ip_len = htons (IP4_HDRLEN + UDP_HDRLEN + lSize);
		...
		// Length of UDP datagram (16 bits): UDP header + UDP data
  		udphdr.len = htons (UDP_HDRLEN + lSize);

  		// UDP checksum (16 bits)
  		udphdr.check = udp4_checksum (iphdr, udphdr, buffer, lSize);
		...
		memcpy (packet + IP4_HDRLEN + UDP_HDRLEN, buffer, lSize * sizeof (uint8_t));
		...
		if ((sd = socket (AF_INET, SOCK_RAW, IPPROTO_RAW)) < 0) {
  		...
  		// Send packet.
  		if (sendto (sd, packet, IP4_HDRLEN + UDP_HDRLEN + lSize, 0, (struct sockaddr *) &sin, sizeof (struct sockaddr)) < 0)  {
